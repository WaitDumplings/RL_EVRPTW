...
class StateEVRP(NamedTuple):
    ...
    def __my_getitem__(self, key):
        ...
        return self._replace(
            ...
            chrg_battery=self.chrg_battery[key],
        )
    ...
    def initialize(input, visited_dtype=torch.uint8):
        ...
        min_battery, _ = torch.min(0.3 * torch.sqrt(torch.sum((RS_coords - Customer_coords) ** 2, dim=-1)), dim=1)
        chrg_battery = torch.zeros(depot.shape[0], 1)
        ...
        return StateEVRP(
            ...
            chrg_battery=chrg_battery.to(all_nodes.device),
    ...
    def update(self, selected):
        ...
        # chrg battery
        chrg_battery = torch.zeros_like(selected_battery).to(selected_battery.device)
        battery_mask = (prev_a >= (nodes_num - n_loc))
        chrg_battery[~battery_mask] += (self.VEHICLE_BATTERY - (self.used_battery + selected_battery))[~battery_mask]  # remain battery at RSs       
        used_battery = ((self.used_battery + selected_battery) * battery_mask).float()
        ...
        return self._replace(
            ...
            lengths=lengths, used_battery=used_battery, chrg_battery = chrg_battery, cur_coord=cur_coord, i=self.i + 1
        )
